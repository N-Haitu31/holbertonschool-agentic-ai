...
- **Task 3** (`worker.js`, `docker-compose.yml`): a Human-in-the-Loop Pre-Hook — a refund (`status: "refund"`) suspends the Worker until an operator answers `o`/`n` in the terminal (`readline/promises`, `stdin_open` + `tty` in Compose). The decision is scored in Langfuse (`1` authorized, `0` refused) on the same trace as the analysis. A FinOps budget guard also alerts and scores (`finops_budget`) any analysis that exceeds a per-call or cumulative token budget (configurable by environment variable). Each human decision also records the operator's name (`OPERATOR_NAME`, declarative, never guessed: `non-identifié` if unset) in the score comment.
