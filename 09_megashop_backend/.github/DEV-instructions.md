...
- **Image Docker complète** : chaque fichier nécessaire à l'exécution d'un service (point d'entrée, modules `require`s, etc.) doit être copié dans l'image. Un `npm test` local qui passe ne prouve pas que le conteneur démarre : tu dois construire l'image et démarrer réellement le service avant de dire que c'est terminé.
- **Cohérence des modules** : respecte le système de modules du projet (CommonJS avec `require` ou ESM avec `import`) sans mélanger les deux syntaxes, y compris quand tu t'inspires d'un exemple écrit dans l'autre.
- **Dépendances installées** : après toute modification de `package.json`, lance `npm install` avant de lancer les tests, pour que l'environnement local corresponde à ce que déclare le fichier.
